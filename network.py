...
Группа: <МТС02-19-01>
ФИО: Коновалова Оксана Сергеевна
"""""""""""""""""""""""""""""""""""""""
# Библиотеки
...
class Network(object):  # используется для описания нейронной сети
    def __init__(self, sizes):  # конструктор класса
        # self – указатель на объект класса сети сети
    ...
    def SGD(  # Стохастический градиентный спуск
        ...
        for j in range(epochs):  # цикл по эпохам выборки
    ...
    def update_mini_batch(  # Шаг градиентного спуска
        ...
        for x, y in mini_batch:
            delta_nabla_b, delta_nabla_w = self.backprop(x,
                                                         y)  # послойно вычисляем градиенты
            # dC / db и dC / dw для текущего прецедента(x, y)
            nabla_b = [nb + dnb for nb, dnb in zip(nabla_b,
                                                   delta_nabla_b)]  # суммируем градиенты dC / db
            # для различных прецедентов текущей подвыборки
            nabla_w = [nw + dnw for nw, dnw in zip(nabla_w,
                                                   delta_nabla_w)]  # суммируем градиенты dC / dw
            # для различных прецедентов текущей подвыборки
    ...
    def backprop(  # Алгоритм обратного распространения
        ...
        # для каждого слоя (первоначально заполняются нулями)
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            activation = sigmoid(z)  # считаем выходные сигналы
            # текущего слоя, применяя сигмоидальную функцию активации к активационным потенциалам слоя
            activations.append(activation)  # добавляем элемент (выходные сигналы слоя) в конец списка

        # обратное распространение
        delta = self.cost_derivative(activations[-1], y) * sigmoid_prime(zs[-1])  # считаем меру
        # влияния нейронов выходного слоя L на величину ошибки (BP1)
        ...
        for l in range(2, self.num_layers):
            ...
            delta = np.dot(self.weights[-l + 1].transpose(), delta) * sp  # считаем меру влияния
            # нейронов l - го слоя на величину ошибки(BP2)
